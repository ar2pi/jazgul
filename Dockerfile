...
RUN \
  ...
  yum install -y wget patch tar bzip2 unzip openssh-clients && \
  ...
  `# Install common npm packages: gulp, bower, browser-sync` \
...
#COPY vhosts/api.conf /etc/nginx/hosts.d/
#COPY vhosts/website.conf /etc/nginx/hosts.d/
#COPY vhosts/api.conf /data/conf/nginx/hosts.d/
#COPY vhosts/website.conf /data/conf/nginx/hosts.d/
